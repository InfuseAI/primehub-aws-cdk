...
const username =
  app.node.tryGetContext('username') ||
  process.env.USERNAME ||
  'dev@infuseai.io';
const name = app.node.tryGetContext('name') || process.env.NAME || 'cdk';
const email = app.node.tryGetContext('email') || '';
const primehubMode =
  app.node.tryGetContext('primehubMode') || process.env.PRIMEHUB_MODE || 'ee';
const basedDomain =
  app.node.tryGetContext('basedDomain') || process.env.AWS_BASED_DOMAIN || '';
const primehubVersion = app.node.tryGetContext('primehubVersion') || null;
const primehubPassword =
  app.node.tryGetContext('primehubPassword') ||
  process.env.PH_PASSWORD ||
  crypto.randomBytes(32).toString('hex');
const keycloakPassword =
  app.node.tryGetContext('keycloakPassword') ||
  process.env.KC_PASSWORD ||
  crypto.randomBytes(32).toString('hex');
...
const enforceUpdatePassword =
  (app.node.tryGetContext('enforceUpdatePassword') || 'false') === 'false'
    ? false
    : true;
const cpuDesiredCapacity = parseInt(
  app.node.tryGetContext('cpuDesiredCapacity') || '0',
  10
);
const gpuDesiredCapacity = parseInt(
  app.node.tryGetContext('gpuDesiredCapacity') || '0',
  10
);
const cpuMaxCapacity = parseInt(
  app.node.tryGetContext('cpuMaxCapacity') || '2',
  10
);
const gpuMaxCapacity = parseInt(
  app.node.tryGetContext('gpuMaxCapacity') || '2',
  10
);
const scaleDownDelay = parseInt(
  app.node.tryGetContext('scaleDownDelay') || '10',
  10
);
