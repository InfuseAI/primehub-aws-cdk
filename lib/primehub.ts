...
export class PrimeHub extends cdk.Construct {
  constructor(scope: cdk.Construct, id: string, props: PrimeHubProps) {
    ...
    const graphqlSecretKey =
      scope.node.tryGetContext('GraphqlSecretKey') ||
      process.env.ADMIN_UI_GRAPHQL_SECRET_KEY ||
      crypto.randomBytes(32).toString('hex');
    const hubProxySecretKey =
      scope.node.tryGetContext('HubProxySecretKey') ||
      process.env.HUB_PROXY_SECRET_TOKEN ||
      crypto.randomBytes(32).toString('hex');

    const enabledPrimeHubStore = props.primehubStoreBucket ? true : false;
    const instanceTypes = [
      PrimeHub.generateInstanceTypeConfig(props.cpuInstance),
      PrimeHub.generateInstanceTypeConfig(props.gpuInstance),
    ].flat();
...
KEYCLOAK_DEPLOY=true`;
    ...
    mkdirSync(`./artifact/${props.clusterName}/helm_override`, {
      recursive: true,
    });
    writeFileSync(
      `./artifact/${props.clusterName}/helm_override/primehub.yaml`,
      primehubHelmOverride.toString()
    );
    ...
    const bucket = new s3.Bucket(
      this,
      `${props.primehubConfigBucket}-s3-bucket`,
      {
  ...
  static generateInstanceTypeConfig(instanceType: string): HelmValues[] {
    ...
    const [cpu, memory, gpu] = this.calculateInstanceSize(
      instanceClass,
      instanceSize
    );
